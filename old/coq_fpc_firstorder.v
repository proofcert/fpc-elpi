(* 
   This file contains the legacy implementation based on treating Coq terms
   as purely first order proof certificates. It is not fully updated, and
   current developement focuses on a kernel providing dependent types.
*)
...
(*
   ...
   Note that proof terms generated by coq-elpi may need to be normalized prior
   to being returned, for example by assignment to a fresh Elpi-level variable
   (or by printing, conversion, etc.). This is a well-known issue that currently
   lives on in Elpi.
*)
