...
Consider a conjecture of the form
...
%
In our current setting $P$ and $Q$ will be \lsti{Inductive}
...
intuitionistic interpretation, which is close to what Coq supports.  % proofs of formulas such as
% \[
%   \exists x [(\tau(x)\wedge P(x)) \wedge \neg Q(x)]
%   \tag{*}\label{eq:full}
% \]
...
\end{lstlisting}
The tactic will call \lsti{check} with the given FPC on the dependent variables and will simply call a vanilla meta-interpreter (see Fig.~\ref{fig:interp} in the appendix) to test the negation of the property:
\begin{lstlisting}
...
against data \lsti{x,xs} generated exhaustively via proofs bounded by
at most $5$ unfoldings of the library \lsti{Inductive} definitions of
