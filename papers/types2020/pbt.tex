...
~\cite{QChick}, in particular of relational specifications, is still
very labor intensive. We do not intend to compete with
\textsf{QuickChick}, but we shall see that we can test immediately
\lsti{Inductive} definitions that corresponds to pure Horn
programs. We can do that without having to provide a decidability
proof for those definitions. Furthermore, we are not committed to a
fixed random generation strategy, which in gnereal requires additional
...
and Booleans with if-then-else and test-for-zero, which comes from the \emph{Software Foundations} book series (\url{https://softwarefoundations.cis.upenn.edu/plf-current/Types.html}):
\begin{lstlisting}
Inductive tm : Type :=
