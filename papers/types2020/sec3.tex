...
applications of hypotetic assumptions. This means, at the term level, that we
...
Systems we have the same grammar for terms and types, as shown in figure~\ref{fig:terms}.
...
The syntax depends on the external set of inductive definitions available in
the context: we denote by $\Pscr_A$ the set of type declarations for the
...
Figure~\ref{fig:augmented} contains a simple proof system \dots in which each
...
certificates.% (in the case of $\vee$ and $\exists$).
%
%
%
\todo{Explain relations with other term assignments. Note that we do not use
explicit substitutions (we are cut free). Explain the choice of not having
certificates represent proof terms. Explain that the unfold clause handles
...
The following is the signature of the term constructors we use:
\begin{lstlisting}
  kind term   type.
...
The constructor for products forms a term from a term and a \lP function from
terms to terms; when, in the implementation of the product-left rule, we
apply the second argument of \lsti{prod _ Ty1 Ty2} to the variable \lsti{Tm},
we get a term that can be used to continue backchaining. The meaning of this
application is meta-level substitution, in the style of Higher Order Abstract
