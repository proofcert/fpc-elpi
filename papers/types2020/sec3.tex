...
\begin{metanote}
  (AM): I would start with the logical reconstruction of \texttt{check}, adding the LJT-style proof terms to fig \ref{fig:augmented} and then reverse engineer what we want/need to say about focusing and FPC. We \textbf{do} not talk about PBT yet, just logic programming (with proof terms). In sec 4 we package \texttt{check} in a PBT query and explain how \texttt{check} works for term generation.
\end{metanote}
...
\begin{metanote}
  add proof terms and backchaining to sequents and code
\end{metanote}
\begin{figure}
...
\begin{metanote}
  Rethink and update (with the right connectives) these FPC
\end{metanote}
\begin{figure}
