...
%\begin{footnotesize} % No effect on typesetting
\begin{figure}[t]
  %
%  \renewcommand{\Gamma}{}
...
\infer{\XXi\vdash  s : A}
{%(A,s) \in \mathcal{A}
  {E[] \vdash_{CIC} s : A}% The order should be s : A, not A : s ?
}
\qquad
  \infer{\bc {\XXi}{} {[]} A A}{\trueExpert{\XXi}}{}\qquad
%  \infer{\bc{\forall x.\, D} A}{\bc{D[t/x]} A}{}\qquad
  \infer{\bc{\XXi}{}{(t\ ::\ l)} {\forall x:B.\, D} A}
  {\XXi_1\vdash t : A\qquad \bc{\XXi_2}{}l {B[t/x]} A\qquad\prodExpert{\XXi}{\XXi_1}{\XXi_2}}{}
\]
...
\todo{Reput safe-dest-app in code, a bit more details about the unfold case}
...
\todo{explain Ev = Term as an Elpi thing}
...
more customable that the one offered by the \lsti{auto}'s hard-wired
\lsti{Debug} facility.
%
%%% Local Variables:
%%% mode: latex
