...
denotes the number of parameters and playes no role. We delegate to
...
at the term level that we do not have variables in our grammar of
...
constructors of an \lsti{Inductive} definition from the global
environment, one that matches the head symbol of the atom we aim to backchain on, and then
call the latter judgment that will find a correct instantiation, if any. The rules for backchaining are standard,
...
will use in this case. We just need to call \lsti{check} on the goal,
...
\lsti{reflexivity}; % as well as the modularity of using
...
\lsti{Hints} via Elpi's \lsti{Db} feature, and FPCs can provide a
\emph{trace} that may be more customizable than the one offered by the
\lsti{(e)auto}'s hard-wired \lsti{Debug} facility. 
