...
lkf_entry Cert CForm Term :- trans- CForm IForm, coq.say IForm, ljf_entry Cert {{lp:IForm -> fatom}} Term.

trans- A B :- coq.say "trans-" A B, read X, fail.
trans+ A B :- coq.say "trans+" A B, read X, fail.
% nnf {{True}} {{True}}.
% nnf {{False}} {{False}}.
...
trans+ (app [global Ex, Ty, (fun _ Ty B)]) (app [global Ex, Ty, (fun _ Ty B')]) :-
  coq.locate "ex" Ex,
  pi x\ trans+ (B x) (B' x).
trans+ N         {{lp:N' -> fatom}} :-  isNeg N, trans- N N'.
...
trans- P          {{lp:P' -> fatom}}  :- isPos P, trans+ P P'.
trans- {{lp:A \/ lp:B}}  {{lp:A' /\ lp:B'}} :- trans- A A', trans- B B'. 
trans- {{lp:A /\ lp:B}}  {{lp:A' \/ lp:B'}} :- trans- A A', trans- B B'.
trans- {{lp:A -> lp:B}}  {{lp:A' /\ lp:B'}} :- trans+ A A', trans- B B'.
